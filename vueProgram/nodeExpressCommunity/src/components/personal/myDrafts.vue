<template>
 ...
 <ul class="garagraph clearfix"> 
  <li v-for="(expressArctile,index) in draftsArctileList" :key=index> 
...
export default {
  name:'myDrafts',
  created: function() {
    this.getPerDraftsList(this.index.getSession("username"));
  }, 
  components: {
    pg
  },
  computed: {
    ...mapState(["draftsArctileList","indexTotalNum",])
  },
  methods: {
    ...mapActions(["getPerDraftsList","changeDraftsPage"]),
    del(args) {
      this.plugs.deleteMessage(args);
    },
    pagechange(currentPage) {
      this.changeDraftsPage(currentPage);
    },
  },
  store
}
