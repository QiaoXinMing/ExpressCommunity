<template>
 ...
 <ul class="garagraph clearfix"> 
  <li v-for="(expressArctile,index) in questionArctileList" :key=index> 
              ...
              <div  class="beautiful-article-info-content right">
                <p>{{expressArctile.arctile_content}}</p>
                 <div class="right"> 
                    ...
                    <span class="delete" @click="del($event)">删除</span>
                  </div>
                </div>
              </div>
            </div>
...
export default {
  ...
  created: function() {
    this.getPerQuestionList(this.index.getSession("username"));
  }, 
  components: {
    pg
  },
  computed: {
    ...mapState(["questionArctileList","indexTotalNum"])
  },
  methods: {
    ...mapActions(["getPerQuestionList","changeQuestionPage"]),
    del(args) {
      this.plugs.deleteMessage(args);
    },
    pagechange(currentPage) {
     this.changeQuestionPage(currentPage);
    },
  },
  store
};
