<template>
 ...
 <ul class="garagraph clearfix"> 
  <li v-for="(expressArctile,index) in answerArctileList" :key=index> 
...
export default {
  ...
  created: function() {
        this.getPerAnswerList(this.index.getSession("username"));
  }, 
  components: {
    pg
  },
  computed: {
    ...mapState(["answerArctileList","indexTotalNum"])
  },
  methods: {
    ...mapActions(["getPerAnswerList","changeAnswerPage"]),
    del(args) {
      this.plugs.deleteMessage(args);
    },
    pagechange(currentPage) {
     this.changeAnswerPage(currentPage);
    },
  },
  store
};
